...
interface RadialOrbitalTimelineProps {
  ...
  /** Called when a node is clicked. If provided, suppresses the expand-card behaviour. */
  onNodeClick?: (item: TimelineItem) => void;
  /** Tailwind class(es) for the outermost container. Defaults to full-screen. */
...
export default function RadialOrbitalTimeline({
  timelineData,
  onNodeClick,
  containerClassName = "w-full h-screen flex flex-col items-center justify-center bg-black overflow-hidden",
  ...
  const toggleItem = (id: number) => {
    ...
    // If external handler provided, navigate instead of expanding
    if (onNodeClick) {
      onNodeClick(item);
      return;
    }
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          {/* Centre orb */}
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-[#0D7680] via-[#990F3D] to-[#0D7680] animate-pulse flex items-center justify-center z-10">
              ...
              >
                ...
                <div
                  className={`
                    ...
                    hover:scale-125 hover:border-white hover:bg-white hover:text-black
                  `}
                >
                  <Icon size={16} />
                </div>
                ...
                {/* Expanded card — only shown in standalone mode (no onNodeClick) */}
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-center">
                        <Badge className={`px-2 text-xs ${getStatusStyles(item.status)}`}>
                          {item.status === "completed" ? "ACTIVE" : item.status === "in-progress" ? "MODERATE" : "QUIET"}
                        </Badge>
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      <div className="mt-4 pt-3 border-t border-white/10">
                        <div className="flex justify-between items-center text-xs mb-1">
                          <span className="flex items-center"><Zap size={10} className="mr-1" />Activity</span>
                          <span className="font-mono">{item.energy}%</span>
                        </div>
                        <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-[#0D7680] to-[#990F3D]" style={{ width: `${item.energy}%` }} />
                        </div>
                      </div>
                      {item.relatedIds.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-white/10">
                          <div className="flex items-center mb-2">
                            ...
                            <h4 className="text-xs uppercase tracking-wider font-medium text-white/70">Related Sectors</h4>
                          </div>
