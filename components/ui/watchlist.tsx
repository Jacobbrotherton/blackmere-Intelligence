...
import { Plus, X, Eye, TrendingUp, TrendingDown, Loader2, Lock } from "lucide-react";
...
export function Watchlist() {
  ...
  const addTicker = () => {
    // If limit already reached — user is trying to use again, lock
    ...
    fetchQuote(t);
    setInput("");
    // Consume use AFTER adding ticker — lock the input, not the ticker list
    if (!isPremium) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 shadow-xl shadow-black/20">
      ...
      {/* Add ticker input OR locked state — ticker list stays visible either way */}
      {!isPremium && limitReached ? (
        <div className="mb-5 flex items-center justify-between gap-4 bg-zinc-800 border border-zinc-700 rounded-xl p-4">
